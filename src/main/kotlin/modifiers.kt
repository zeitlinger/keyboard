...
fun modifierTypes(s: String): Map<HomeRowType, LayerName?> =
    s
        .split(",")
...
private fun modTapKey(
    ...
    when {
        key in translator.noHoldKeys -> {
            throw IllegalStateException("key $key not allowed for mod tap at $pos")
        }
                ...
                .also {
                    if (!key.isNo) {
                        throw IllegalStateException(
                            "key $key not allowed for one shot modifier at $pos",
                        )
                    }
                }
        }
...
fun addCustomIfNotSimpleKey(
    key: QmkKey,
    pos: KeyPosition,
    translator: QmkTranslator,
): QmkKey =
    if (key.key.contains("(") || translator.nonSimpleKeys.containsKey(key.key)) {
        translator.ignoreMissing.add(key)
