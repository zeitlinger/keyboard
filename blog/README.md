...
- **[`after-n.yaml`](after-n.yaml)** — the full picture: red keys are adaptives rewriting ordinary
  keys (`x`/`h`/`r`/`p` → `l`/`n`/`g`/`k`; `p` is a combo, so it shows as the red `k` badge), and the
