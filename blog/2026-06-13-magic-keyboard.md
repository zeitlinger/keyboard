...
Working title (B): **The Secret World of Keyboard Wizardry** — leads with the rabbit hole, not the
personal build, since the "fascinating niche" angle is the stronger hook for a cold Medium audience.
Subtitle: *"I built a keyboard whose keys finish my words. Here's the rabbit hole that taught me how —
and how you'd start your own."*

Alternatives if B doesn't land:

- **A (focus on created keyboard):** *How I Built My Magic Keyboard Layout* —
  subtitle *"Open-door day at the school of keyboard wizardry: an outsider-to-archmage tour of a rabbit
  hole you didn't know existed."*
- **C (curiosity-bait):** *My Keyboard Finishes My Words Before I Do* — subtitle *"An outsider's tour of
  the strange world of keyboard layouts, told through the 34-key board I built at the bottom of the
  rabbit hole."*

### Still to do

- **Pictures**: layout diagrams are in (auto-generated via keymap-drawer; the keyboard repo's
  `mise run generate` produces them). The real Ferris Sweep photo is in (Part 2). Three 🖼️ placeholders
  remain for the content team — search the doc for `🖼️`:
  1. Hero — the school-of-wizardry castle (intro).
  2. Awkward finger motions (Part 1, Qwerty).
  3. Slab vs. split comparison (Part 1, shape).
  - The 4/8-bit codec diagram is in (`images/codec.png`, source `images/codec.svg`). The getreuer
    magic-key figure was dropped — the real keyboard photo covers it.
  - Also still wanted: GIFs/short clips of magic + adaptive keys rewriting text live.
- **SFB number**: done — the Part 2 expert box cites Hands Down Vibranium's base SFB (0.57%) vs
  Qwerty (4.39%) from Cyanophage. Trigram metrics (redirects etc.) deliberately omitted as out of
  scope ("archmage bookkeeping"). No bespoke analyzer needed.
- **Series or single post?** Natural split if it runs long: Parts 1–4 as the post, the Appendix as a
  follow-up. Decide after feedback.
