...
with a tax: in that curl, the top row became an awkward stretch for my pinky and index. That's
partly down to my own finger lengths, and yours might reach there just fine, so I stopped
...
hundred words and phrases stored as plain text, one byte per character, would push it over. But
...
just enough to keep the firmware under the line. *That's* the archmage
