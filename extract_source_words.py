...
def count_words(roots: list[Path], *, min_length: int, max_length: int) -> Counter[str]:
...
def write_output(output: Path, rows: list[tuple[int, float, str]]) -> None:
    lines = ["# count\tfreq\tword"]
    lines.extend(f"{count}\t{freq:.12f}\t{word}" for count, freq, word in rows)
    output.write_text("\n".join(lines) + "\n")


def main() -> None:
    args = parse_args()
    counts = count_words(args.roots, min_length=args.min_length, max_length=args.max_length)
