...
static const uint8_t chord_data[] = {
    ...
    0x05, 0x51, 0x1d, 0xb3, 0x00, 0x06, 0xd1, 0x1d, 0x80, 0x26, 0x06, 0x62, 0x38, 0x28, 0x1c, 0x05,
    0xc8, 0x8c, 0x7a, 0x00, 0x0a, 0x6a, 0x34, 0x40, 0x4e, 0x10, 0xa3, 0x01, 0x08, 0x62, 0xb8, 0x90,
...
int chord_transition(int state, uint16_t keycode) {
    switch (state) {
                ...
                // n
                case 88:
                    if (keycode == KC_A) return 89;
                    if (keycode == KC_C) return 90;
                    if (keycode == KC_D) return 91;
...
void chord_output(int state) {
    switch (state) {
        ...
        case 89: chord_decode_send(chord_data + 410); break;
                // nc
        case 90: chord_decode_send(chord_data + 415); break;
                // nd
        case 91: chord_decode_send(chord_data + 420); break;
                // nh
